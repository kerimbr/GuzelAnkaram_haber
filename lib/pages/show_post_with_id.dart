...
class ShowPostWithId extends StatefulWidget {



  /*
  * Bildirime Tıklanınca Açılan Sayfa
  */

  String postId;
  BuildContext myContext;

  ShowPostWithId({@required this.postId,this.myContext});

  @override
  _ShowPostWithIdState createState() => _ShowPostWithIdState();
}

class _ShowPostWithIdState extends State<ShowPostWithId> {
  ...
  Widget build(BuildContext context) {
    BuildContext myContext = widget.myContext;
    ViewModel _viewmodel = Provider.of<ViewModel>(myContext);
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      children: <Widget>[
                        ...
                        SizedBox(width: 5),
                        Text(
                          "${news.date.day > 10 ? news.date.day : "0" +
                              news.date.day.toString()} / "
                              "${news.date.month > 10 ? news.date.month : "0" +
                              news.date.month.toString()} / "
                              "${news.date.year}",
                        ...
                        SizedBox(width: 10),
                        FutureBuilder<bool>(
                            future: _checkPinned(news.id,myContext),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                                return IconButton(
                                  icon: !pinned
  ...
  Future<bool> _checkPinned(String id,BuildContext mycontext) async{
    ViewModel _viewmodel = Provider.of<ViewModel>(mycontext,listen: false);
  ...
  Future<Post> _future(String id,BuildContext mycontext) async{

    ViewModel _viewmodel = Provider.of<ViewModel>(mycontext,listen:false);

    Post post;
    // Göderi Detaylarını Al
    post = await _viewmodel.getPostId(id);
    // Gönderi Görselini Al
